...
interface ImageUploaderProps {
  onImageCropped: (croppedImage: string, description: string) => void;
  onCancel: () => void;
}
...
const ImageUploader: React.FC<ImageUploaderProps> = ({ onImageCropped, onCancel }) => {
  ...
  // 1. Maneja la subida de la imagen
  ...
  // 2. Dibuja la imagen en el canvas cuando se carga
  useEffect(() => {
  ...
  // 3. Lógica de selección del recorte
  ...
  // 4. Recorta y guarda la imagen
  const handleRecortar = () => {
    if (!imageOriginal || !lienzoRef.current) return;
    ...
    if (ctxTemp) {
      ...
      onImageCropped(croppedImageUrl, description);
    }
  };
